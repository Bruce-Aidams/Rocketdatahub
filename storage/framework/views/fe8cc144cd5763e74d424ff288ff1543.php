    ...
    <div class="space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5"><?php echo e($orders->total()); ?> total orders</p>
                </div>
            </div>
        ...
        <?php if(request('user_id')): ?>
            ...
            <?php if($filteredUser): ?>
                <div class="flex items-center gap-3 bg-primary/5 border border-primary/10 px-4 py-2.5 rounded-2xl w-fit">
                    ...
                    <p class="text-sm font-black text-slate-900 dark:text-white"><?php echo e($filteredUser->name); ?>'s Orders</p>
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="flex flex-wrap gap-2 p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl w-fit">
                ...
                ?>
                <?php $__currentLoopData = $statusMap; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button onclick="filterBy('status', '<?php echo e($val); ?>')"
                        class="px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition-all whitespace-nowrap
                                                                                                                            <?php echo e($activeStatus === $val ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'); ?>">
                        <?php echo e($label); ?>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <select onchange="filterBy('per_page', this.value)"
                    ...
                    <?php $__currentLoopData = [10, 25, 50, 100, 500]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($val); ?>" <?php echo e(request('per_page', 10) == $val ? 'selected' : ''); ?>><?php echo e($val); ?> / page
                        </option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-3">
            ...
            <div class="p-4 rounded-2xl border shadow-sm flex items-center justify-between cursor-pointer hover:scale-[1.02] active:scale-95 transition-all
                                                    <?php echo e($activeNetwork === 'all' ? 'border-primary ring-2 ring-primary/30 bg-primary/5 text-primary' : 'border-primary/20 bg-primary/5 text-primary'); ?>"
                onclick="filterBy('network', 'all')">
                <div>
                    <p class="text-[10px] font-black uppercase tracking-widest opacity-60">Total</p>
                    <p class="text-2xl font-black tabular-nums"><?php echo e($totalFilteredOrders); ?></p>
                </div>
            ...
            <?php $__currentLoopData = $networks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $net): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <?php if($count > 0): ?>
                    <div class="p-4 rounded-2xl border shadow-sm flex items-center justify-between cursor-pointer transition-all
                                                                                                                        <?php echo e($isActive ? 'ring-4 scale-[1.02]' : 'hover:scale-[1.02] active:scale-95'); ?>"
                        style="<?php echo e($cardStyle); ?> <?php echo e($isActive ? 'ring-color:rgba(255,255,255,0.4);' : ''); ?>"
                        onclick="filterBy('network', '<?php echo e($net); ?>')">
                        <div>
                            <p class="text-[10px] font-black uppercase tracking-widest opacity-70"><?php echo e($net); ?></p>
                            <p class="text-2xl font-black tabular-nums"><?php echo e($count); ?></p>
                        </div>
                        <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="<?php echo e($iconStyle); ?>">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
        ...
        <div
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                        <?php $__empty_1 = true; $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors group"
                                id="order-row-<?php echo e($order->id); ?>">

                                
                                <td class="px-5 py-3.5">
                                    <input type="checkbox" name="order_ids[]" value="<?php echo e($order->id); ?>"
                                ...
                                <td class="px-5 py-3.5">
                                    ...
                                    <p class="text-[10px] text-slate-400 mt-0.5"><?php echo e($order->created_at->format('d M, H:i')); ?>
                                ...
                                <td class="px-5 py-3.5">
                                    ...
                                    <p class="text-[10px] font-mono text-slate-400 mt-0.5"><?php echo e($order->recipient_phone); ?></p>
                                </td>
                                ...
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="px-2 py-0.5 rounded text-[10px] font-black uppercase"
                                            style="<?php echo e($badgeStyle ?: 'background:#e2e8f0;color:#475569;'); ?>"><?php echo e($order->bundle->network ?? '?'); ?></span>
                                        <span
                                            class="text-sm font-semibold text-slate-800 dark:text-slate-200"><?php echo e($order->bundle->name ?? '—'); ?></span>
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-3.5 text-right">
                                    <p class="text-sm font-black text-slate-900 dark:text-white tabular-nums">
                                        GH₵<?php echo e(number_format($order->cost, 2)); ?></p>
                                </td>
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    ...
                                    ?>
                                    <select onchange="updateStatus(<?php echo e($order->id); ?>, this.value)"
                                        class="px-2 py-1 rounded-lg text-[10px] font-bold border outline-none cursor-pointer transition-all <?php echo e($sc); ?>">
                                        <?php $__currentLoopData = ['pending_payment', 'awaiting_transfer', 'validation', 'processing', 'delivered', 'failed']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <option value="<?php echo e($st); ?>" <?php echo e($order->status === $st ? 'selected' : ''); ?>>
                                                <?php echo e($statusLabels[$st] ?? ucwords(str_replace('_', ' ', $st))); ?>
                                ...
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <?php if($order->status === 'failed'): ?>
                                            <button onclick="retryOrder(<?php echo e($order->id); ?>)" title="Retry Processing"
                                        ...
                                        <a href="<?php echo e(route('admin.orders.edit', $order->id)); ?>" title="Edit Order"
                                        ...
                                        <button onclick="deleteOrder(<?php echo e($order->id); ?>)" title="Delete Order"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center gap-3 text-slate-400 dark:text-slate-600">
                                        ...
                                        <?php if(request()->anyFilled(['status', 'network', 'search', 'user_id'])): ?>
                                            <a href="<?php echo e(route('admin.orders')); ?>"
            ...
            <?php if($orders->isNotEmpty()): ?>
                <div class="px-5 py-3.5 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between gap-4">
                    ...
                    <?php echo e($orders->withQueryString()->links('pagination::simple-tailwind')); ?>
    ...
    <?php $__env->startPush('scripts'); ?>
        <script>
            ...
            function downloadSelected() {
                ...
                const url = new URL('<?php echo e(route("admin.orders.export")); ?>');
            ...
            async function executeBulkAction(action) {
                ...
                try {
                    const response = await fetch('<?php echo e(route("admin.orders.bulk_action")); ?>', {
                        method: 'POST',
                        headers: {
            ...
            async function retryOrder(orderId) {
                try {
                    const response = await fetch(`<?php echo e(url('admin/orders')); ?>/${orderId}/retry`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                            'Accept': 'application/json'
            ...
            function exportAll() {
                const url = new URL('<?php echo e(route("admin.orders.export", request()->all())); ?>');
                window.location.href = url.href;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\bruce\OneDrive\Desktop\Projects\RocketDataHub\resources\views/admin/orders/index.blade.php ENDPATH**/ ?>