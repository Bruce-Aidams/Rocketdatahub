        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm p-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-50 dark:border-slate-800 pb-4 mb-4">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs text-slate-500 font-medium">Last audited: <?php echo e($auditStats['last_audit'] ?? 'Never'); ?></span>
                </div>
            </div>

            <?php if($auditStats['unbalanced']): ?>
                <div class="flex items-start gap-4 p-4 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-800/40 rounded-2xl">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-rose-600 dark:text-rose-400/80 mt-1">
                            The cumulative transaction ledger does not balance with current wallet allocations. Mismatch Amount: <strong class="font-bold">GHS <?php echo e(number_format($auditStats['mismatch_amount'], 2)); ?></strong>. Please run <code>php artisan audit:wallets</code> for complete details.
                        </p>
                    </div>
                </div>
                
                <?php if(!empty($auditStats['audit_logs'])): ?>
                <div class="mt-4 overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800 text-xs">
                            <?php $__currentLoopData = $auditStats['audit_logs']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>
                                <td class="py-3 font-medium text-slate-900 dark:text-white">ID <?php echo e($log['user_id']); ?> - <?php echo e($log['name']); ?></td>
                                <td class="py-3 text-right text-slate-700 dark:text-slate-300">GHS <?php echo e(number_format($log['current_balance'], 2)); ?></td>
                                <td class="py-3 text-right text-slate-700 dark:text-slate-300">GHS <?php echo e(number_format($log['ledger_balance'], 2)); ?></td>
                                <td class="py-3 text-right font-bold text-rose-600">GHS <?php echo e(number_format($log['difference'], 2)); ?></td>
                            </tr>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Bruce\Desktop\Projects\cloudtech\resources\views/admin/financials/index.blade.php ENDPATH**/ ?>