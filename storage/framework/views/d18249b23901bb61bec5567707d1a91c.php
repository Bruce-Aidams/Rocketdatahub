...
<head>
    ...
    <title><?php echo e($reseller->settings['storefront_title'] ?? ($reseller->store_name ?? $reseller->name) . "'s Data Hub"); ?> - <?php echo e(config('app.name')); ?></title>
    <?php echo app('Illuminate\Foundation\Vite')(['resources/css/app.css', 'resources/js/app.js']); ?>
    <link href="https://fonts.googleapis.com/css2?family=Geist:wght@100..900&display=swap" rel="stylesheet">
    <script defer src="https://unpkg.com/alpinejs@3.x.x/dist/cdn.min.js"></script>
    <?php if(isset($reseller->settings['storefront_theme_color'])): ?>
    <style>
        :root {
            --primary: <?php echo e($reseller->settings['storefront_theme_color']); ?> !important;
            --color-primary: <?php echo e($reseller->settings['storefront_theme_color']); ?> !important;
        }
    </style>
    <?php endif; ?>
</head>

<body class="antialiased bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-screen" style="<?php echo e(isset($reseller->settings['storefront_bg_color']) ? 'background-color: ' . $reseller->settings['storefront_bg_color'] . ' !important;' : ''); ?>">
    <div class="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-20 space-y-8 md:space-y-16">
        
        <div class="text-center space-y-4 md:space-y-6">
            <div
                class="inline-flex items-center gap-3 px-4 py-2 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase tracking-[0.2em] animate-in fade-in slide-in-from-top-4 duration-1000" style="<?php echo e(isset($reseller->settings['storefront_theme_color']) ? 'color: ' . $reseller->settings['storefront_theme_color'] . '; background-color: ' . $reseller->settings['storefront_theme_color'] . '1a;' : ''); ?>">
                <span class="relative flex h-2 w-2">
                    <span
                        class="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75" style="<?php echo e(isset($reseller->settings['storefront_theme_color']) ? 'background-color: ' . $reseller->settings['storefront_theme_color'] . ';' : ''); ?>"></span>
                    <span class="relative inline-flex rounded-full h-2 w-2 bg-primary" style="<?php echo e(isset($reseller->settings['storefront_theme_color']) ? 'background-color: ' . $reseller->settings['storefront_theme_color'] . ';' : ''); ?>"></span>
                </span>
                Official Reseller Store
            </div>
            <?php if(!empty($reseller->settings['storefront_logo_url'])): ?>
                <div class="flex justify-center my-2 animate-in fade-in zoom-in duration-700">
                    <img src="<?php echo e($reseller->settings['storefront_logo_url']); ?>" alt="<?php echo e($reseller->store_name ?? $reseller->name); ?> Logo" class="h-20 w-20 object-contain rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 bg-white p-2">
                </div>
            <?php endif; ?>
            <h1
                ...
                <?php echo e($reseller->settings['storefront_title'] ?? ($reseller->store_name ?? $reseller->name) . "'s Data Hub"); ?>
            ...
            <p
                ...
                <?php echo e($reseller->settings['storefront_description'] ?? 'Select your preferred network and bundle size below to receive instant data delivery. Secure, fast, and reliable.'); ?>
