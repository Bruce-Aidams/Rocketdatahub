...
<div class="auth-viewport" x-data="{
    ...
    <div class="w-full max-w-[440px] flex items-center justify-between px-2 mb-6">
        <a href="<?php echo e(route('login')); ?>" class="w-10 h-10 rounded-full bg-white dark:bg-slate-900 shadow-sm flex items-center justify-center border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
    ...
    <div class="auth-mobile-card">
        ...
        <?php if(session('status')): ?>
            <div class="mb-4 p-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 rounded-2xl text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider"><?php echo e(session('status')); ?></div>
        <?php endif; ?>
        <?php if($errors->any()): ?>
        ...
        <form method="POST" action="<?php echo e(route('password.email')); ?>" x-ref="forgotForm" @submit.prevent="submit" class="space-y-6">
            <?php echo csrf_field(); ?>
            ...
            <div class="text-center pt-2">
                <a href="<?php echo e(route('login')); ?>" class="inline-flex items-center gap-2 text-xs font-black uppercase tracking-wider text-slate-400 hover:text-primary transition-colors">
