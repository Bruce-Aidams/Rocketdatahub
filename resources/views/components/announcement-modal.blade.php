@if($announcement)
    <div x-data="{ 
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl w-full max-w-md relative z-10 border border-slate-100 dark:border-slate-800 overflow-hidden" 
            ...
            <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between {{ $bgClass }}">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-2xl bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm">
                        @if($announcement->type === 'info')
                            <svg class="w-5 h-5 {{ $iconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    ...
                    <div>
                        ...
                        <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">{{ $announcement->created_at->format('M d, Y') }}</p>
                    </div>
                </div>
            ...
            <div class="p-6">
                <h4 class="text-lg font-bold text-slate-900 dark:text-white mb-2 leading-tight">{{ $announcement->title }}</h4>
