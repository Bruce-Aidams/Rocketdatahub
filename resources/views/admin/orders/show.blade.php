                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-3xl font-black tracking-tight text-slate-900 dark:text-slate-100 leading-tight">
                                {{ $order->bundle->name ?? 'Bundle Archived' }}
                            </p>
                            <div class="flex items-center gap-3 mt-3">
                                <span
                                    class="inline-flex px-3 py-1 rounded-lg bg-slate-900 dark:bg-primary text-[10px] font-black text-white uppercase tracking-[0.2em]">{{ $order->bundle->network ?? 'N/A' }}</span>
                        ...
                        <div class="text-right">
                            <span
                                ...
                                Placed</span>
                            <span
                                class="font-black text-sm text-slate-600 dark:text-slate-400">{{ $order->created_at->format('M d, Y') }}</span>
                ...
                <div class="grid grid-cols-3 gap-6 relative z-10">
                    <div class="p-6 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
                        ...
                        <p class="text-2xl font-black tracking-tight tabular-nums">GHC
                            {{ number_format($order->cost_price, 2) }}</p>
                    </div>
                    <div class="p-6 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
                        <span class="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 block mb-2.5">Selling
                            Price</span>
                        <p class="text-2xl font-black tracking-tight tabular-nums">GHC {{ number_format($order->cost, 2) }}
                        </p>
                    </div>
                    <div
                        ...
                        <span class="text-[10px] font-black uppercase tracking-[0.2em] text-white/80 block mb-2.5">Net
                            Profit</span>
                        <p class="text-2xl font-black tracking-tight tabular-nums">GHC
                            {{ number_format($order->cost - $order->cost_price, 2) }}</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <form action="{{ route('admin.orders.update', $order->id) }}" method="POST" class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="status"
                                ...
                                <option value="validation" {{ $order->status === 'validation' ? 'selected' : '' }}>Validating
                                </option>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div
                        ...
                        <span class="font-black text-xs text-slate-900 dark:text-slate-100 tabular-nums">GHC
                            {{ number_format($order->user->wallet_balance, 2) }}</span>
                    </div>
                    <div
                        class="p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-2xl text-center">
                        <span
                        ...
                        <span
                            class="font-black text-xs text-slate-900 dark:text-slate-100 tabular-nums">{{ $order->user->orders()->count() }}</span>
                    </div>
                </div>
