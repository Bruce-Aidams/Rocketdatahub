                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                @endphp
                                <div id="events-data" data-available="{{ json_encode($availableEvents) }}" class="hidden"></div>
                                @foreach($availableEvents as $event)
                                    <label
                                        ...
                                        <input type="checkbox" name="webhook_events[]" value="{{ $event }}" {{ in_array($event, $savedEvents) ? 'checked' : '' }}
    ...
    if(response.ok) window.dispatchEvent(new CustomEvent('toast', { detail: { message: 'Webhook configuration synchronized!', type: 'success' } }));
        ...
        <div x-show="tab === 'docs'" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 md:p-12 border border-slate-100 dark:border-slate-800 shadow-sm text-center">
                ...
                <a href="{{ route('admin.docs') }}" class="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:opacity-90 shadow-xl shadow-indigo-500/20 transition-all">
